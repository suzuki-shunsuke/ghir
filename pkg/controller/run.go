...
func (c *Controller) Run(ctx context.Context, logger *slog.Logger, input *InputRun) error {
	...
	for _, release := range rs {
		...
		if err := c.input.GitHub.EditRelease(ctx, "owner", "repo", release.DatabaseID, release.Description+"\n"); err != nil {
		...
		if err := c.input.GitHub.EditRelease(ctx, "owner", "repo", release.DatabaseID, release.Description); err != nil {
